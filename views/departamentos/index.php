...
<head>
    ...
    <title>Mantenimiento de Departamentos - sgdoc</title>
...
<body class="bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-200 min-h-screen flex">
    ...
    <main class="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <header class="h-14 px-8 flex items-center border-b border-slate-200/50 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-900/80 backdrop-blur-md shrink-0 z-20">
            <div class="flex items-center gap-2 text-[9px] font-black uppercase tracking-[0.3em] text-slate-400">
                ...
                <span class="text-slate-300">›</span>
                <span class="text-primary">Departamentos</span>
            </div>
        ...
        <div class="flex-1 overflow-y-auto px-12 py-10 space-y-10">
            ...
            <div class="flex justify-between items-end">
                <div class="space-y-1">
                    <h1 class="text-[32px] font-black text-slate-custom dark:text-white tracking-tight">Departamentos</h1>
                    <p class="text-[11px] text-slate-400 font-medium italic">Gestione los departamentos de la institución para organizar el direccionamiento de solicitudes.</p>
                </div>
            </div>
            ...
            <div class="space-y-6">
                <div class="flex items-center justify-between px-4">
                    <h2 class="text-xs font-black text-slate-400 uppercase tracking-widest">Catálogo de Departamentos</h2>
                ...
                <div class="bg-white dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 rounded-[2.5rem] shadow-sm overflow-hidden flex flex-col">
                    <div class="overflow-x-auto custom-scrollbar flex-1">
                        <table class="w-full text-left table-fixed min-w-[800px]">
                            ...
                            <tbody id="departamentos-table-body" class="divide-y divide-slate-50 dark:divide-slate-900">
                                <?php if (!empty($departamentos)): ?>
                                    <?php foreach ($departamentos as $d): ?>
                                    <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-900/50 transition-colors group departamento-row">
                                        <td class="px-8 py-6 align-middle">
                                            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">DEP-<?= str_pad($d['id'], 3, '0', STR_PAD_LEFT) ?></p>
                                        </td>
                                        <td class="px-8 py-6 align-middle">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <p class="text-[11px] font-black text-slate-700 dark:text-white uppercase tracking-tight line-clamp-2"><?= htmlspecialchars($d['nombre']) ?></p>
                                            </div>
                                        </td>
                                        <td class="px-8 py-6 align-middle">
                                            <p class="text-[11px] text-slate-400 dark:text-slate-500 font-medium truncate" title="<?= htmlspecialchars($d['descripcion'] ?? '') ?>">
                                                <?= htmlspecialchars($d['descripcion'] ?: 'Sin descripción') ?>
                                            </p>
                                        </td>
                                        <td class="px-8 py-6 align-middle text-center">
                                            <span class="inline-flex text-[9px] font-black px-2.5 py-1 rounded-full uppercase tracking-widest <?= $d['activo'] == 1 ? 'text-emerald-600 bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-100/20' : 'text-slate-400 bg-slate-100 dark:bg-slate-800' ?>">
                                        ...
                                        <td class="px-8 py-6 align-middle text-right">
                                            <div class="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all transform group-hover:translate-x-0 translate-x-2">
                                                <button onclick='editarDepartamento(<?= json_encode($d) ?>)' class="size-9 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-800 rounded-xl flex items-center justify-center text-slate-400 hover:text-primary transition-all shadow-sm" title="Editar Departamento">
                                                    <span class="material-symbols-outlined text-lg">edit</span>
                                                </button>
                                                <form method="POST" action="/departamentos/estado/<?= $d['id'] ?>" style="display:inline;">
                                                    <?= \App\Core\Security::csrfInput() ?>
                                                    <button type="submit" class="size-9 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-800 rounded-xl flex items-center justify-center text-slate-400 hover:text-<?= $d['activo'] == 1 ? 'secondary' : 'emerald-500' ?> transition-all shadow-sm" title="<?= $d['activo'] == 1 ? 'Desactivar' : 'Activar' ?>">
                                                        <span class="material-symbols-outlined text-lg"><?= $d['activo'] == 1 ? 'block' : 'undo' ?></span>
                                                    </button>
                                                </form>
                                            </div>
                                        </td>
                                ...
                                <?php else: ?>
                                    <tr>
                                        <td colspan="5" class="py-12 text-center text-slate-400">
                                            ...
                                            <p class="text-[10px] font-bold uppercase tracking-widest">No hay departamentos registrados</p>
                                        </td>
                                    </tr>
                                <?php endif; ?>
                            </tbody>
                    ...
                    <div class="flex justify-between items-center pt-6 pb-6 border-t border-slate-50 dark:border-slate-900 px-4">
                        <p class="text-[9px] font-black text-slate-300 uppercase tracking-widest italic">Mostrando <?= count($departamentos) ?> de <?= $totalRegistros ?> departamentos</p>
                    ...
                    <?php else: ?>
                    <div class="flex justify-between items-center pt-6 pb-6 border-t border-slate-50 dark:border-slate-900 px-4">
                        <p class="text-[9px] font-black text-slate-300 uppercase tracking-widest italic">Mostrando todos los departamentos (<?= $totalRegistros ?? count($departamentos) ?>)</p>
                    </div>
                    <?php endif; ?>
                </div>
            </div>
        ...
        <div id="modal-nuevo" class="fixed inset-0 z-50 hidden items-center justify-center p-6 bg-white/60 dark:bg-slate-950/60 backdrop-blur-xl">
            <div class="bg-white dark:bg-slate-900 w-full max-w-sm rounded-[2.5rem] shadow-2xl p-10 space-y-8 relative">
                ...
                <div class="space-y-1">
                    <h3 class="text-xl font-black text-slate-custom dark:text-white uppercase tracking-tight">Nuevo Departamento</h3>
                    <p class="text-[9px] text-slate-400 font-bold uppercase tracking-widest">Registrar nueva unidad en el catálogo</p>
                </div>
                <form action="/departamentos/guardar" method="POST" class="space-y-4">
                    <?= \App\Core\Security::csrfInput() ?>
                    <div class="space-y-1">
                        <label class="text-[8px] font-black text-slate-400 uppercase tracking-widest ml-1">Nombre</label>
                        <input type="text" name="nombre" class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border-none rounded-2xl text-[11px] font-bold focus:ring-1 focus:ring-primary/20 transition-all outline-none" placeholder="EJ. DEPARTAMENTO DE COMPRAS" required>
                    </div>
                    <div class="space-y-1">
                    ...
                    <button type="submit" class="w-full bg-primary text-white py-4 rounded-2xl font-black uppercase text-[10px] tracking-[0.2em] shadow-lg shadow-teal-900/20">Crear Departamento</button>
                </form>
            </div>
        </div>
        ...
        <div id="modal-editar" class="fixed inset-0 z-50 hidden items-center justify-center p-6 bg-white/60 dark:bg-slate-950/60 backdrop-blur-xl">
            <div class="bg-white dark:bg-slate-900 w-full max-w-sm rounded-[2.5rem] shadow-2xl p-10 space-y-8 relative">
                ...
                <div class="space-y-1">
                    <h3 class="text-xl font-black text-slate-custom dark:text-white uppercase tracking-tight">Editar Departamento</h3>
                    <p class="text-[9px] text-slate-400 font-bold uppercase tracking-widest">Modificar datos existentes</p>
                </div>
                <form action="/departamentos/actualizar" method="POST" class="space-y-4">
                    <?= \App\Core\Security::csrfInput() ?>
    ...
    <script>
        ...
        // Buscador
        document.getElementById('search-departamentos')?.addEventListener('input', function(e) {
            const term = e.target.value.toLowerCase();
            const rows = document.querySelectorAll('#departamentos-table-body .departamento-row');
